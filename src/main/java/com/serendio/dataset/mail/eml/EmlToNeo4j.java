...
public class EmlToNeo4j {
	...
	public void ingestEml(String inputPath) throws MessagingException,
			IOException {
	...
	public static void showFiles(File[] files, EmailDocToNeo4j neo4jInterface)
		...
		for (File file : files) {
			if (file.isDirectory()) {
				...
				showFiles(file.listFiles(), neo4jInterface); // Calls same
																// method again.
			} else {
				// mailCounter++;
				filecount++;
				...
				if (filecount > 0) {
					...
					try {
						...
						BufferedWriter bufferedWriter = new BufferedWriter(
								writer);
						bufferedWriter.append("FileCount:" + filecount + "\t"
